...
const getRoleFromEmail = (email: string) => {
  ...
  if ([
    ...
    'mme.ruh.ac.lk',
    'cee.ruh.ac.lk'
  ].includes(domain)) return 'Staff Member';
...
const UserProfile: React.FC<UserProfileProps> = ({ userProfile }) => {
  ...
  const getFullDepartment = (dept: string) => {
    ...
    if (d.includes('electrical')) return 'Department of Electrical and Information Engineering';
    if (d.includes('mechanical')) return 'Department of Mechanical and Manufacturing Engineering';
    if (d.includes('civil')) return 'Department of Civil and Environmental Engineering';
    return dept;
  };
          ...
          {userProfile ? (
            <React.Fragment>
              <div className="user-profile-title">
                {userProfile.title ? `${userProfile.title} ` : ''}{userProfile.firstname} {userProfile.lastname}
              </div>
