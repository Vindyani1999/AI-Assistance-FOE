...
from .utils.preference_learner import PreferenceLearner
...
class MLRecommendationEngine:
    ...
    def __init__(self, db: Session = None, config: Optional[RecommendationConfig] = None) -> None:
        ...
        logger.info("Initializing MLRecommendationEngine with MySQL integration")
        ...
        try:
            ...
            for room_data in room_utilization:
                ...
                if conflicts == 0:
                    ...
                    status_text = "Available" if conflicts == 0 else "Currently Booked"
...
class MLRecommendationEngineFactory:
    
    @staticmethod
    def create_engine(config: RecommendationConfig = None, 
                     environment: str = None) -> MLRecommendationEngine:
        if config is None:
            try:
        ...
        return MLRecommendationEngine(config=config)
    
    @staticmethod
    def create_development_engine() -> MLRecommendationEngine:
        """Create engine for development environment"""
        return MLRecommendationEngineFactory.create_engine(environment='development')
    
    @staticmethod
    def create_production_engine() -> MLRecommendationEngine:
        """Create engine for production environment"""
        return MLRecommendationEngineFactory.create_engine(environment='production')
    
    @staticmethod
    def create_testing_engine() -> MLRecommendationEngine:
        """Create engine for testing environment"""
        return MLRecommendationEngineFactory.create_engine(environment='testing')


def create_recommendation_engine_with_fallback(
    db: Session = None, 
    ...
    try:
        return MLRecommendationEngine(db=db, config=config)
        ...
        try:
            fallback_config = config or RecommendationConfig()

            # IMPORTANT: db must stay None (do NOT replace with config inside engine)
            return MLRecommendationEngine(db=None, config=fallback_config)
