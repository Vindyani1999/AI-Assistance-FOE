...
class SimilarityEngine:
    ...
    def __init__(self, db: Session, cache_manager: CacheManager):
        ...
        self.room_similarity_weights = {
            'capacity': 0.25, 'area': 0.15, 'features': 0.20,
            'usage_patterns': 0.25, 'user_overlap': 0.15
        }
        
        self.time_similarity_weights = {
            'hour_proximity': 0.30, 'day_similarity': 0.20,
            'duration_match': 0.25, 'usage_patterns': 0.25
        }
    ...
    def find_similar_rooms(self, target_room_id: int, limit: int = 10,
        ...
        try:
            rooms = self.db.query(MRBSRoom).filter(
                and_(MRBSRoom.disabled == False, MRBSRoom.id != target_room_id)
            ).all()
    ...
    def _create_empty_similarity_score(self, entity1, entity2, similarity_type):
    ...
    def _get_room_profile(self, room_id: int) -> Optional[RoomProfile]:
        ...
        try:
            ...
            bookings = self.db.query(MRBSEntry).filter(
                and_(MRBSEntry.room_id == room_id, MRBSEntry.start_time >= six_months_timestamp)
            ).all()
    ...
    def _calculate_time_usage_similarity(self, profile1: TimeSlotProfile, profile2: TimeSlotProfile) -> float:
    ...
    def _calculate_room_preference_similarity(self, bookings1: List[MRBSEntry], bookings2: List[MRBSEntry]) -> float:
    ...
    def _calculate_time_preference_similarity(self, bookings1: List[MRBSEntry], bookings2: List[MRBSEntry]) -> float:
    ...
    def _calculate_duration_preference_similarity(self, bookings1: List[MRBSEntry], bookings2: List[MRBSEntry]) -> float:
    ...
    def _calculate_booking_frequency_similarity(self, bookings1: List[MRBSEntry], bookings2: List[MRBSEntry]) -> float:
    ...
    def _calculate_room_similarity_confidence(self, room1: RoomProfile, room2: RoomProfile) -> float:
    ...
    def _calculate_time_similarity_confidence(self, profile1: TimeSlotProfile, profile2: TimeSlotProfile) -> float:
    ...
    def find_best_alternative_room(self, original_room_id: int, 
        ...
        try:
            ...
            similar_rooms = self.find_similar_rooms(
                original_room_id, limit=20, min_similarity=0.2, context=booking_context
            )
            ...
            for similarity_score in similar_rooms:
                ...
                if booking_context.get('prefer_same_area', False):
                    original_room = self.db.query(MRBSRoom).filter(MRBSRoom.id == original_room_id).first()
                    alternative_room = self.db.query(MRBSRoom).filter(MRBSRoom.id == room_id).first()
                ...
                if user_id:
                    user_bookings = self.db.query(MRBSEntry).filter(
                        and_(MRBSEntry.create_by == user_id, MRBSEntry.room_id == room_id)
                    ).count()
    ...
    def calculate_booking_similarity(self, booking1: Dict[str, Any], 
        ...
        try:
            ...
            if 'room_id' in booking1 and 'room_id' in booking2:
                room_sim = self.calculate_room_similarity(booking1['room_id'], booking2['room_id'])
            ...
            if 'user_id' in booking1 and 'user_id' in booking2:
                if booking1['user_id'] == booking2['user_id']:
                    ...
                    user_sim = self.calculate_user_booking_similarity(booking1['user_id'], booking2['user_id'])
