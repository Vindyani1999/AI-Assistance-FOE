...
class PatternAnalyzer:
        ...
        try:
            ...
            for row in result:
                bookings.append({
                    ...
                    'description': row.description or '', 'create_by': row.create_by, 
    ...
    def _analyze_duration_patterns(self, bookings: List[Dict[str, Any]]) -> Dict[str, Any]:
        durations = [int((booking['end_time'] - booking['start_time']).total_seconds() / 60) 
    ...
    def _analyze_advance_booking(self, bookings: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        style = 'last_minute' if avg_advance < 1 else \
                'short_term' if avg_advance < 7 else \
                'medium_term' if avg_advance < 30 else 'long_term'
    ...
    def detect_anomalies(self, user_id: str, new_booking: Dict[str, Any]) -> Dict[str, Any]:
        try:
            patterns = self.analyze_user_patterns(user_id)
            anomalies = []
            
            booking_hour = new_booking.get('start_time', datetime.now()).hour
            preferred_hours = patterns.get('preferred_hours', {}).get('preferred_hours', [])
            if preferred_hours and booking_hour not in preferred_hours:
                anomalies.append({
                    'type': 'unusual_time',
                    'severity': 'low',
                    'message': f'Booking at {booking_hour}:00 is outside your usual hours'
                })
            
            booking_duration = (new_booking.get('end_time', datetime.now()) - 
                              new_booking.get('start_time', datetime.now())).total_seconds() / 60
            avg_duration = patterns.get('meeting_duration_patterns', {}).get('average_duration', 60)
            if abs(booking_duration - avg_duration) > avg_duration * 0.5:
                anomalies.append({
                    'type': 'unusual_duration',
                    'severity': 'medium',
                    'message': f'Duration of {booking_duration}min differs from your average of {avg_duration}min'
                })
            ...
            return {'has_anomalies': False, 'anomalies': [], 'risk_score': 0}
    
    def predict_next_booking(self, user_id: str) -> Dict[str, Any]:
        try:
            patterns = self.analyze_user_patterns(user_id)
            
            preferred_hours = patterns.get('preferred_hours', {}).get('preferred_hours', [9])
            peak_hour = preferred_hours[0] if preferred_hours else 9
            
            preferred_days = patterns.get('preferred_days', {}).get('preferred_days', [0])
            peak_day = preferred_days[0] if preferred_days else 0
            
            avg_duration = patterns.get('meeting_duration_patterns', {}).get('average_duration', 60)
            
            preferred_rooms = patterns.get('room_preferences', {}).get('preferred_rooms', [])
            likely_room = preferred_rooms[0] if preferred_rooms else None
            ...
            predicted_start = predicted_date.replace(hour=peak_hour, minute=0, second=0, microsecond=0)
            predicted_end = predicted_start + timedelta(minutes=int(avg_duration))
            
            return {
                'predicted_start_time': predicted_start.isoformat(),
                'predicted_end_time': predicted_end.isoformat(),
                'predicted_room': likely_room,
                'confidence': min(patterns.get('total_bookings', 0) / 20.0, 1.0),
                'based_on_bookings': patterns.get('total_bookings', 0)
            }
            
        except Exception as e:
            logger.error(f"Error predicting next booking: {e}")
            return {
