    ...
    def _calculate_end_time(self, start_time: str, duration_hours: float) -> str:
        try:
            if isinstance(start_time, str):
                ...
                elif ':' in start_time and len(start_time.split(':')) >= 2:
...
Provide alternatives that are available for the FULL {duration} hour duration.

JSON format:
{{"alternatives_analysis": [{{"room_name": "Alt", "recommendation_strength": "high", "reasons": ["reason1"], "explanation": "Why", "trade_offs": ["gain", "lose"]}}],
"timing_alternatives": [{{"suggested_start_time": "09:00", "suggested_end_time": "10:00", "explanation": "Better time"}}],
...
JSON format:
{{"optimal_times": [{{"time_slot": "09:00-10:00", "efficiency_score": 0.9, "reasoning": "Why optimal"}}],
"room_suggestions": [{{"room_name": "Room", "utilization_score": 0.3, "recommendation": "Why good"}}],
"scheduling_tips": ["Tip"], "conflict_predictions": ["High demand 2-3 PM"], "efficiency_score": 0.8}}"""
...
Provide 1-2 sentence explanation focusing on benefits and user patterns."""
    ...
    def _get_fallback_room_recommendations(self, request_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for i, room in enumerate(available_rooms[:3]):
            recommendations.append({
                ...
                'score': 0.6 - (i * 0.1), 
                'reason': f'Available room with suitable capacity',
                'suggestion': {
    ...
    def _enhance_alternatives_fallback(self, alternatives: List[Dict[str, Any]], original_request: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        fallback_explanations = [
            ...
            f"Similar features, available for full {duration}h", 
            f"Available at suitable time with {duration}h duration", 
            f"Suitable for your meeting requirements", 
            f"Recommended based on capacity and availability"
        ]
            ...
            'optimal_times': [
                {
                    ...
                    'reasoning': f'Morning hours typically less busy'
                },
                {
                    'start_time': '14:00', 
                    'end_time': self._calculate_end_time('14:00', duration),
                    'efficiency_score': 0.7, 
                    'reasoning': f'Good afternoon option'
                }
            ],
            'room_suggestions': [{
                'room_name': 'Room Name', 
                'utilization_score': 0.3, 
                'recommendation': f'Generally available for {duration}h'
            }],
            'scheduling_tips': [
                f'Book {duration}h slots in advance', 
                f'Consider morning slots for higher availability'
            ],
    ...
    def _get_fallback_explanation(self, recommendation: Dict[str, Any]) -> str:
        ...
        if rec_type == 'alternative_time':
            return f"Consider {room_name}{time_info} - it offers good availability."
        elif rec_type == 'alternative_room':
            return f"{room_name} is available{time_info} with similar features."
        elif rec_type == 'proactive':
            return f"Based on your history, {room_name}{time_info} is a great choice."
        else:
            confidence_text = "highly" if score > 0.8 else "moderately" if score > 0.6 else "reasonably"
            return f"{room_name} is {confidence_text} recommended{time_info}."
    ...
    def get_processor_status(self) -> Dict[str, Any]:
        return {
            ...
            'features': ['end_time_calculation', 'duration_validation', 'time_range_validation']
        }